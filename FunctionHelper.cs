...
namespace FunctionApp.ECOM_TO_D365.Customer
{
    public class FunctionHelper
    {
            ...
            while (!cancelationToken.IsCancellationRequested)
            {
                ...
                Parallel.ForEach(messages, new ParallelOptions { MaxDegreeOfParallelism = requestParams.MaxDegreeOfParallelism }, message =>
                {
                    ...
                    try
                    {
                        ...
                        var rckCustomer = JsonConvert.DeserializeObject<RCKCustomer>(originalMessage);

                        ProcessCustomer(requestParams, rckCustomer, telemetryClient, stage, startDate, cloudDb, blob);
        ...
        private static void ProcessCustomer(CreateCustomerParams requestParams, RCKCustomer rckCustomer, TelemetryClient telemetryClient, EDI_STAGING stage, DateTime startDate, ICloudDb cloudDb, IBlob blob)
        {
            var updation = bool.Parse(rckCustomer.IsUpdate);
            ...
            var erpCustomer = rckCustomer.ErpCustomers.FirstOrDefault();
            var Is_CustomerAffiliation = bool.Parse(erpCustomer.isCustomerAffiliation);
            var emails = rckCustomer.ErpCustomers.Select(c => c.Email).ToList();
            var cosmosCustomers = GetCustomersFromCosmos(cloudDb, emails);
            var ErpCustomerAccountNumber = cosmosCustomers.Select(c => c.ErpCustomerAccountNumber).ToList();

            SplitAddressesIncaseOfSameAddressForBothBillingAndShipping(rckCustomer.ErpCustomers, cosmosCustomers);

            if (updation)
            {
                telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_TO_D365_CUSTOMER, $"Transaction Id: {stage.Transaction_Id}, Updating customers {string.Join(",", rckCustomer.ErpCustomers.Select(c => c.EcomCustomerId))} to D365!");

                var (success, response) = CustomerUpdateToAXAsync(rckCustomer.ErpCustomers, requestParams, telemetryClient, stage, cloudDb, startDate).GetAwaiter().GetResult();
                if (Is_CustomerAffiliation)
                {
                    erpCustomer.AccountNumber = ErpCustomerAccountNumber.FirstOrDefault();
                    var c_response = CustomerAffiliationUpdateToAXAsync(rckCustomer.ErpCustomers, requestParams, telemetryClient, stage, cloudDb, startDate).GetAwaiter().GetResult();
                ...
                if (response != null)
                {
                    ...
                    var magentoCustomers = rckCustomer.ErpCustomers.Where(c => axResponseLogs.Any(l => l.MagentoCustomerId == c.EcomCustomerId)).ToList();
                    ...
                    var existingCosmosCustomers = cosmosCustomers.Where(c => requestEmails.Contains(c.Email) && requestStores.Contains(c.StoreId)).ToList();
            ...
            else
            {
                ...
                var cosmosCustomer = cosmosCustomers.Where(c => c.Email == erpCustomer.Email && c.StoreId == erpCustomer.StoreId).FirstOrDefault();
                ...
                if (cosmosCustomer != null)
                {
                    ...
                    CustomerController.SendCustomerResponseToEcom(cosmosCustomer, rckCustomer, telemetryClient, requestParams.MagentoResponseServiceUri, requestParams.MagentoResponseServiceToken, stage.Transaction_Id);
                ...
                else
                {
                    ...
                    if (Is_CustomerAffiliation)
                    {
                        ...
                        var response = CustomerAffiliationUpdateToAXAsync(rckCustomer.ErpCustomers, requestParams, telemetryClient, stage, cloudDb, startDate).GetAwaiter().GetResult();
                    }
                    ...
                    CustomerController.SendCustomerResponseToEcom(customerResponse, rckCustomer.ErpCustomers.FirstOrDefault(), telemetryClient, requestParams.MagentoResponseServiceUri, requestParams.MagentoResponseServiceToken, stage.Transaction_Id);
        ...
        private static void SplitAddressesIncaseOfSameAddressForBothBillingAndShipping(List<ErpCustomer> erpCustomers, List<CosmosCustomer> cosmosCustomers)
        {
            foreach (var erpCustomer in erpCustomers)
            {
                ...
                var cosmosCustomer = cosmosCustomers?.FirstOrDefault(c => c.EcomCustomerId.ToString() == erpCustomer.EcomCustomerId && c.StoreId == erpCustomer.StoreId);
